...
| `src/load_dotenv.rs` (referenced from the V1 `config.rs`/`arguments.rs`/... DISCARD row) | load `.env` into the recipe environment | Just's dotenv crate integration, `Config`, `Settings` | DISCARD | ADAPT | A hand-rolled ~60-line parser (`src/dotenv.rs`) reads `KEY=VALUE` lines (comments, quoting) from a `.env` next to the source file, gated by a new `@LoadEnv` file pragma (reusing the existing `@SchemaFile` pragma mechanism, not a Just concept). Loaded values never override an already-set real environment variable; a task's own `env: {}` always wins over both. |
