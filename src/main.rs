...
use spar::{
    ...
    resolver::{GlobalEntry, Resolver, SymbolTable},
...
fn emit_json(result: &spar::evaluator::EvalResult, symbols: &SymbolTable) -> String {
    let val = build_emit_json(result, symbols);
    serde_json::to_string_pretty(&val).unwrap_or_else(|_| "{}".to_string())
}

fn build_emit_json(result: &spar::evaluator::EvalResult, symbols: &SymbolTable) -> serde_json::Value {
    let mut root = serde_json::Map::new();

    // Exported globals only
    for (name, value) in &result.globals {
        let exported = symbols.globals.get(name).map(|entry| {
            match entry {
    ...
    let mut section_keys: Vec<&Vec<String>> = result
        .sections
        .keys()
        .filter(|p| p.len() == 1)
        .collect();
    ...
    for path in section_keys {
        let private = symbols.sections.get(path)
            .map(|e| e.private)
            .unwrap_or(false);
        if !private {
            let name = &path[0];
...
fn build_section_value(path: &[String], result: &spar::evaluator::EvalResult) -> serde_json::Value {
...
fn config_value_to_json(val: &spar::evaluator::ConfigValue) -> serde_json::Value {
    use spar::evaluator::ConfigValue;
    match val {
        ...
        ConfigValue::List(vs) => serde_json::Value::Array(
            vs.iter().map(config_value_to_json).collect()
        ),
