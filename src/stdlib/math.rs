...
pub(crate) fn register(registry: &mut NativeRegistry) {
    registry
        .register(NativeFunction::sync(
            ...
            true,
            |_context, args| {
                int_arg(args, 0, "value")?
                    .checked_abs()
                    .map(Value::Int)
                    .ok_or_else(|| error("abs overflows Spar int"))
            },
        ))
    ...
    for (name, op) in [
        ...
        registry
            .register(NativeFunction::sync(
                ...
                move |_context, args| {
                    Ok(Value::Float(op(
                        float_arg(args, 0, "left")?,
                        float_arg(args, 1, "right")?,
                    )))
                },
            ))
    ...
    registry
        .register(NativeFunction::sync(
            "nativeMath",
            "minInt",
            vec![("left", SparType::Int), ("right", SparType::Int)],
            SparType::Int,
            true,
            |_context, args| {
                Ok(Value::Int(
                    int_arg(args, 0, "left")?.min(int_arg(args, 1, "right")?),
                ))
            },
        ))
    ...
    registry
        .register(NativeFunction::sync(
            ...
            |_context, args| {
                Ok(Value::Int(
                    int_arg(args, 0, "left")?.max(int_arg(args, 1, "right")?),
                ))
            },
        ))
