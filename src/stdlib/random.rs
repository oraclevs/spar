...
pub(crate) fn register(registry: &mut NativeRegistry) {
    registry.register(NativeFunction::sync(
        "nativeRandom", "bytes", vec![("count", SparType::Int)], SparType::Named("Bytes".into()), true,
        |_context, args| {
            ...
            getrandom::getrandom(&mut bytes).map_err(|e| error(format!("OS random source failed: {e}")))?;
            Ok(Value::Bytes(bytes))
        },
    )).expect("nativeRandom::bytes registration must be unique");
    registry.register(NativeFunction::sync(
        "nativeRandom", "randomInt", vec![("min", SparType::Int), ("max", SparType::Int)], SparType::Int, true,
        |_context, args| {
    ...
    registry.register(NativeFunction::sync(
        "nativeRandom", "randomFloat", vec![], SparType::Float, true,
        |_context, _args| {
            let mut bytes = [0u8; 8];
            getrandom::getrandom(&mut bytes).map_err(|e| error(format!("OS random source failed: {e}")))?;
